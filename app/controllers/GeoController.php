...
class GeoController extends MainController
{
    public function getCountryList()
    {
        ...
        $data = array();
        while (!$dbCountry->dry()) {
            array_push($data, array("id" => $dbCountry->id, "name" => $dbCountry->name, "currency" => $dbCountry->currency, "flag" => $this->f3->get('API_URL') . "assets/img/countries/" . strtolower(Helper::getCountryIso($dbCountry->name_en)) . ".svg"));
            $dbCountry->next();
        }
        $response['data'] = $data;
